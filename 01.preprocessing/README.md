...
データの加工を行うknitfabのplanを作成し、加工・学習・評価の一連のタスクを実行するワークフローを作成します。
...
加工や学習などの一連のタスクをワークフローとして実行するプログラムは、Apache AirflowやAWS Step Functionsなどがありますが、
...
3. 加工処理をknitfabで実行すると、後続の学習、評価まで逐次実行されることを確認
...
4. 学習プログラムはデータが60,000サンプルあることが前提なので、同じデータで60,000サンプルにかさ増しする
...
加工処理を使用しなかった場合、データの変更に対して学習と評価の2つのプログラムを変更する必要があるため、

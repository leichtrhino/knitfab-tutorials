...
これらはタスク同士のつながりを直接定義してワークフローを作成します。

一方、Knitfabはデータに付けられたタグにより実行を管理するタグベースのワークフローの機能を有しています。
タグベースのワークフローは、タグに合致する新たなタスクを作成することで、既存の処理を残したままワークフローを部分的に修正できる特徴があります。
そのため、データのバージョンを更新した際に、ワークフローの改修が一部分で済み、更新前と更新後の両方のデータに対応できる利点があります。

#### チュートリアルの流れ

このチュートリアルでは、データ形式の変更に伴うと加工プログラムの改修とKnitfabのワークフローの変更を題材に、下記の流れで説明します。

1. 源泉となるデータを収集するサーバプログラムを作成してデータを収集
2. データを訓練処理に入力できる形式に変換する加工処理を作成
3. 加工処理をKnitfabで実行すると、後続の訓練、評価まで逐次実行されることを確認
4. データスキーマの変更に合わせて加工処理を追加することで、新たなデータ形式に対応できることを確認
...
このチュートリアルは`qmnist`と同じ形式のデータを作成するための加工処理planを作成します。
...
`${YOUR_KNITFAB_NODE}`、`${PORT}`はKnitfabのGetting startedのものと同じ内容です。
...
出力されたテンプレートを編集します。
`inputs`では特定のバージョン`version:v1.0`のデータのみを取り込むように指定しておきます。
...
planの登録が完了したあとは、源泉データをアップロードして、加工、訓練、評価の順に実行することを確認します。
このとき、データのバージョンを表すタグを設定し、バージョンを明示します(`-t version:v1.0`)。
...
リネージの図から旧バージョンのデータを使用して新バージョンの加工が実行されていないことが確認できます。
Knitfabでは、タグに基づいて実行を管理するため、バージョンタグが一致しないplanは実行が抑止されます。

また、Knitfabは処理を追加することでワークフローを部分的に改修できるため、データ変更前の機能も保持される特徴があります。
これを確認するため、`knit plan graph`コマンドで評価に関連する全てのplanを出力します。

```bash
knit plan graph ${PLAN_ID} | dot -T png -o workflow-graph.png
```

![workflow](workflow-graph.png)

`${PLAN_ID}`は評価planのplan IDです。
出力画像の`image=localhost:30503/knitfab-preprocess:v1.0`と`image=localhost:30503/knitfab-preprocess:v2.0`の両方のplanが`active`なので、
変更前と変更後の両方のデータの入力を受け付けることが分かります。

### むすび

このチュートリアルでは、データの加工用のKnitfab planを作成し、データ変更に対応できるワークフローを作成しました。
タグベースワークフローの機能を有するKnitfabを使うと、データ変更時にワークフローに処理を追加するのみでワークフローを部分的に改修でき、データ変更前の処理も保持される特徴があります。
